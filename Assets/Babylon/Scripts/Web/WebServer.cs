...
namespace BabylonHosting
{
    public static class WebServer
    {
        ...
        public static string Unity { get; private set; }
        public static bool IsStarted { get; private set; }
        public static bool IsSupported { get; private set; }
        public static HttpListener HostListener { get; private set; }
        private const string CompressExtentions = ".js, .bin, .babylon, .babylonbinarymeshdata, .babylonbinarygeometrydata";
        ...
        public static bool Activate(string prefix, string root, int port, string unity)
        {
            WebServer.Root = root;
            WebServer.Unity = unity;
        ...
        static void Listen(object state)
        {
            try
            {
                while (WebServer.HostListener.IsListening)
                {
                    ...
                    if (String.IsNullOrEmpty(WebServer.Unity)) WebServer.Unity = "/";
        ...
        {
            WriteResponse(context, new MemoryStream(buffer));
        }

        static void WriteResponse(HttpListenerContext context, FileInfo path)
        {
            FileInfo info = path;
            context.Response.ContentType = GetMimeType(info);
            // Support Pre-Compressed Static Files
            if (WebServer.CompressExtentions.IndexOf(info.Extension, StringComparison.OrdinalIgnoreCase) >=0) {
                ...
                if (RpcContentEncoding.Contains("gzip")) {
                    ...
                    if (File.Exists(gzipFile)) {
                        context.Response.Headers.Add("Content-Encoding", DecompressionMethods.GZip.ToString().ToLower());
                        info = new FileInfo(gzipFile);
                    }
                }
            }
            Stream file = info.OpenRead();
            WriteResponse(context, file);
            file.Close();
        }
        ...
        static void WriteResponse(HttpListenerContext context, Stream stream)
        {
            ...
            stream.CopyTo(output, CopyToOptions.FlushFinal, CompressionMode.Compress, DecompressionMethods.None, packets);
            output.Close();
        }
